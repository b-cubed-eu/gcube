...
This tutorial documents the second part of the gcube simulation
workflow, viz. the detection process.
...
occurrences.](detection-process_files/figure-html/unnamed-chunk-3-1.png)
...
occurrences equal to 100 (see see tutorial 1 about simulating the
occurrence process).
...
point.](detection-process_files/figure-html/unnamed-chunk-5-1.png)

## Detect occurrences

We have our occurrences, but not all occurrences are generally observed.
The detection of occurrences depends on the detection probability of a
species and the sampling bias (includes both sampling bias and effort).
This process can be simulated using the
...
Each observation will have a detection probability value (=the same for
all observations) and a bias weight depending on its spatial
...
1.  With `"no_bias"`, only the detection probability value will decide
    whether an occurrence is observed or not.
2.  With `"polygon"`, bias weights depend on their location inside or
    outside a given polygon with a certain bias strength. We can
    visualise this using the supporting function
...
occurrences.](detection-process_files/figure-html/unnamed-chunk-11-1.png)

We can say that occurrences on the road have 2x larger probability to be
detected.
...
We see that occurrences on the road have twice the bias weights as the
other occurrences.
...
![Spatial distribution of occurrences with indication of bias weights
using a road polygon for each time
point.](detection-process_files/figure-html/unnamed-chunk-13-1.png)

3.  With `"manual"`, bias weights depend on their location inside grid
    cells of a given grid where each cell has its own value. We can
    visualise this using the supporting function
...
![Spatial grid with manual bias
weights.](detection-process_files/figure-html/unnamed-chunk-16-1.png)
...
We indeed see higher bias weights for occurrences where with higher
...
![Spatial distribution of occurrences with indication of bias weights
using a manual grid for each time
point.](detection-process_files/figure-html/unnamed-chunk-18-1.png)

## Example

Now that we know how the supporting functions work, we can simulate the
...
probability, but this is not the case on the road where few are
detected.
...
for each time
point.](detection-process_files/figure-html/unnamed-chunk-20-1.png)
...
We add coordinate uncertainty to the observations using the
...
You can add a value for all observations or a vector with a single value
for each observation. Lets add 25 meters of uncertainty to each
observation.
...
point.](detection-process_files/figure-html/unnamed-chunk-26-1.png)
