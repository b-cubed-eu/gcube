...
This tutorial documents the third part of the gcube simulation workflow,
viz. the grid designation process.
...
to go through this workflow using default arguments.

The user can change these arguments to allow for more flexibility. As
input, we create a polygon in which we simulate occurrences. It
...
occurrences.](grid-designation-process_files/figure-html/unnamed-chunk-3-1.png)
...
occurrences.](grid-designation-process_files/figure-html/unnamed-chunk-5-1.png)

We can for example sample randomly within the polygon over 6 time points
were we use a random walk over time with an initial average number of
occurrences equal to 100 (see tutorial 1 about simulating the occurrence
process).
...
![Spatial distribution of occurrences within the polygon for each time
point.](grid-designation-process_files/figure-html/unnamed-chunk-7-1.png)

We detect occurrences using a 0.9 detection probability and a bias of
0.1 to detect occurrences on the road (see tutorial 2 about simulating
...
for each time
point.](grid-designation-process_files/figure-html/unnamed-chunk-9-1.png)

We only keep the detected occurrences and add 25 meters of uncertainty
to each observation (see tutorial 2 about simulating the detection
process).
...
uncertainty for each time
point.](grid-designation-process_files/figure-html/unnamed-chunk-11-1.png)
...
cube.](grid-designation-process_files/figure-html/unnamed-chunk-14-1.png)

How does grid designation take coordinate uncertainty into account? The
default is `"uniform"` randomisation where a random point within the
uncertainty circle is taken as the location of the observation. This
point is then designated to the overlapping grid cell. Another option is
`"normal"` where a point is sampled from a bivariate Normal distribution
with means equal to the observation point and the variance equal to
...
within the uncertainty circle. This can be visualised by using these
...
We sample 1000 times using uniform and normal randomisation to look at
...
In the case of uniform randomisation, we see samples everywhere and
evenly spread within the uncertainty circle.
...
randomisation.](grid-designation-process_files/figure-html/unnamed-chunk-20-1.png)

In the case of normal randomisation, we see some samples outside the
uncertainty circle. This should be 0.05 (=1 - `p_norm`) %. We also see
more samples closer to the central point.
...
randomisation.](grid-designation-process_files/figure-html/unnamed-chunk-21-1.png)

If no coordinate uncertainty is provided, the original observation point
is used for grid designation.

## Example

Now we know how to use the randomisation in
[`grid_designation()`](https://b-cubed-eu.github.io/gcube/reference/grid_designation.md).
By default we use uniform randomisation. We create an occurrence cube
for time point 1.
...
For each grid cell (column `cell_code`) at each time point (column
`time_point`), we get the number of observations (column `n`, sampled
within uncertainty circle) and the minimal coordinate uncertainty
(column `min_coord_uncertainty`). The latter is 25 for each grid cell
...
Get sampled points within uncertainty circle by setting
`aggregate = FALSE`.
...
Lets visualise were the samples were taken for time point 1. Note that
no distinction is made between zeroes and `NA` values! Every absence
gets a zero value.
...
![Distribution of random samples within uncertainty circle for time
point
1.](grid-designation-process_files/figure-html/unnamed-chunk-25-1.png)
...
![Distribution of minimal coordinate uncertainty for time points 1 and
2.](grid-designation-process_files/figure-html/unnamed-chunk-26-1.png)
