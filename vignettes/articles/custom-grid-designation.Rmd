...
To perform grid designation, we loop over each species while using `grid_designation()`.
Opt for `lapply()` or `purrr::map()` functions to improve computation time.
...
This is slow in case of big datasets due to the geometry column.
