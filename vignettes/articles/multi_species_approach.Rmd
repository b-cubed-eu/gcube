...
```{r}
# Combine dataframes
multi_species_dataset1 <- cbind.data.frame(taxonomy, multi_species_args)

# How does the dataset look like?
glimpse(multi_species_dataset1)
```

Or we can give the dataframe directly as input to `generate_taxonomy()` and it will create a species with taxonomic hierarchy for each row.
